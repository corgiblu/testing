...
public final class BHTAPI {
    ...
    public static final Function<HurtSourceInfo, Function<CharSequence, HurtSourceInfo.HurtSourceData>> HURT_SOURCE_DATA_FUNCTION = i -> s -> new HurtSourceInfo.HurtSourceData(i);
    ...
    public static LazyOptional<HurtSourceInfo.HurtSourceData> get(LivingEntity entity, DamageSource source) {
        ...
        return HurtProvider.hurt(entity).lazyMap(c ->
                c.getHurtMap().computeIfAbsent(info.sourceName, BHTAPI.HURT_SOURCE_DATA_FUNCTION.apply(info))
        );
    }
}
